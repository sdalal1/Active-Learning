...
def ilqr_iter(x0, u_traj):
    ...
    res = solve_bvp(zp_dyn_list, zp_bc, tlist, np.zeros((8, tsteps)), verbose=1, max_nodes=100)
...
u_traj = np.tile(np.array([0.005, 0.001]), reps=(tsteps, 1))
init_u = np.tile(np.array([0.005, 0.001]), reps=(tsteps, 1))
